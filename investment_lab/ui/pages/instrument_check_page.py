...
def render() -> None:
    st.markdown("## Проверить инструмент")
    disclaimer(SHORT_DISCLAIMER)
    st.markdown("<div class='lab-panel'>Форма проверяет параметры одного инструмента и показывает предварительную оценку справа.</div>", unsafe_allow_html=True)
...
def _bond_tab(kind: str) -> None:
    ...
    with left:
        ...
        nkd = st.number_input("НКД", min_value=0.0, value=12.0, step=1.0, key=f"{kind}_nkd")
        ...
        coupon = st.number_input("Купон, %", min_value=0.0, value=8.0, step=0.1, key=f"{kind}_coupon")
        years = st.number_input("Срок до погашения, лет", min_value=0.1, value=3.0, step=0.1, key=f"{kind}_years")
        frequency = st.selectbox("Периодичность купона", [1, 2, 4, 12], index=1, key=f"{kind}_freq")
        credit_risk = st.number_input("Кредитный риск, %", min_value=0.0, value=0.0 if kind == "ОФЗ" else 2.0, step=0.25, key=f"{kind}_credit")
        liquidity = st.number_input("Ликвидность, дней", min_value=0, value=3 if kind == "ОФЗ" else 15, key=f"{kind}_liq")
        commission = st.number_input("Комиссия, %", 0.0, 100.0, 0.1, key=f"{kind}_com")
        ...
        if kind == "Корпоративная облигация":
            st.selectbox("Рейтинг эмитента", ["AAA", "AA", "A", "BBB", "BB и ниже"], key=f"{kind}_rating")
