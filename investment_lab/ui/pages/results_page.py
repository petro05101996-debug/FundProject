...
from investment_lab.data.mockup_metrics import MOCKUP_RESULTS
...
def render() -> None:
    ...
    display_summary = pd.DataFrame(MOCKUP_RESULTS)
    ...
    with c2: kpi_card("Стресс-результат", f"{leader['stress_value']:,.0f} ₽".replace(",", " "), "Худший стресс")
    with c3: kpi_card("Стресс-просадка", f"{leader['worst_stress_impact_pct']:.1f}%", "По стресс-сценариям")
    with c4: kpi_card("Ликвидность", str(leader["liquidity_label"]), "Сроки выхода")
    with c5: kpi_card("Риск", str(leader["risk_label"]), "Средняя оценка")
    with c6: kpi_card("Сложность", str(leader["complexity_label"]), "Средняя оценка")

    main, side = st.columns([1.55, .75])
    with main:
        table_card("Таблица сравнения сценариев", display_summary[["scenario", "projected_value", "stress_value", "liquidity_label", "risk_label", "max_position_pct", "status"]].round(2))
        st.plotly_chart(scenario_projection_chart(display_summary, st.session_state["investment_lab_assumptions"].horizon_years), use_container_width=True)
