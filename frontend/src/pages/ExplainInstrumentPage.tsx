...
export default function ExplainInstrumentPage(){
  ...
  useEffect(()=>{(async()=>{try{const resp=await api<{items?:Array<{name:string}>}>('/api/instruments/catalog');setCatalog((resp.items||[]).map(x=>x.name));}catch{/* ignore catalog preload errors */}})();},[]);
  ...
  return <div className='grid2'><div className='card'><h2>Объяснить инструмент</h2><div className='row'><input value={q} onChange={e=>setQ(e.target.value)} placeholder='Введите инструмент'/><button className='btn' onClick={()=>load(q)} disabled={loading}>{loading?'Загрузка...':'Найти'}</button></div><div className='row'>{catalog.map(c=><button key={c} className='btn ghost' onClick={()=>load(c)}>{c}</button>)}</div><p className='muted'>Образовательный раздел: сервис не даёт индивидуальных инвестиционных рекомендаций.</p>{error&&<div className='risk-flag'>{error}</div>}</div>
